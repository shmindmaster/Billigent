...
interface ListOptions { limit?: number; page?: number; }
...
class QueryRepository {
  ...
  static getInstance(): QueryRepository {
    if (!QueryRepository.instance) QueryRepository.instance = new QueryRepository();
  ...
  async list(options: ListOptions = {}) {
    ...
    const querySpec = { query: 'SELECT * FROM c WHERE c.type = "query" ORDER BY c.createdAt DESC' };
  ...
  async create(data: { question: string; userId: string; context?: Record<string, any> }): Promise<QueryRecord> {
    await this.ensureInit();
  ...
  async update(id: string, updates: Partial<QueryRecord>): Promise<QueryRecord | null> {
    await this.ensureInit();
    const existing = await this.get(id);
    if (!existing) return null;
    const updated: QueryRecord = { ...existing, ...updates, id, updatedAt: new Date().toISOString() };
