...
router.get('/evidence/:patternId/:encounterId', async (req, res) => {
  ...
  try {
    ...
    // Publish event
    eventPublisher.publish(makeEvent('query_generated', { 
...
router.get('/appeal/:patternId/:encounterId', async (req, res) => {
  ...
  try {
    ...
    let aiAppealDraft;
    try {
      aiAppealDraft = await azureOpenAIService.generateAppealDraft(appealRequest);
    } catch (aiError) {
      console.warn('Azure OpenAI failed, falling back to mock draft:', aiError);
...
router.get('/metrics/snapshot', (_req, res) => {
  ...
  const byOperation: Record<string, { count: number; avgMs: number; p95Ms: number }> = {};
  for (const op of opEvents) {
    const label = (op as any).payload.label;
    if (!byOperation[label]) byOperation[label] = { count: 0, avgMs: 0, p95Ms: 0 };
    const rec = byOperation[label];
    rec.count += 1;
    // incremental average
    rec.avgMs = rec.avgMs + (((op as any).payload.duration_ms - rec.avgMs) / rec.count);
  }
  // compute p95 per operation
  for (const label of Object.keys(byOperation)) {
    const durations = opEvents.filter(o => (o as any).payload.label === label).map(o => (o as any).payload.duration_ms).sort((a,b) => a-b);
    if (durations.length) {
  ...
  try {
    ...
    if (typeof mod.loadNormalizedCitations === 'function') {
      ...
      all.forEach((c: any) => { byTier[c.authorityTier] = (byTier[c.authorityTier] || 0) + 1; });
      // Simple sample coverage for canonical DEN:PAYERA:CO45 + ENC:demo1
      const coverage = mod.computeCitationCoverage?.('DEN:PAYERA:CO45', 'ENC:demo1');
      citations = { total: all.length, byTier, sampleCoverage: coverage };
    }
  } catch (err) {
    citations = { error: 'citation_metrics_unavailable', message: (err as any)?.message };
  }
...
router.post('/conversational', async (req, res) => {
  ...
  try {
    ...
    let aiResponse;
    try {
      aiResponse = await azureOpenAIService.generateConversationalResponse(conversationalQuery);
    } catch (aiError) {
    ...
    // Publish event
    eventPublisher.publish(makeEvent('conversational_query', { 
