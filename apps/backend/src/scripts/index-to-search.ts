...
class FHIRIndexer {
  ...
  async indexFHIRData(): Promise<void> {
      ...
      try {
        ...
        for (let i = 0; i < lines.length; i++) {
          try {
            ...
            console.warn(
              `    ⚠️  Skipping malformed record in ${resourceType}: ${error}`
            );
          }
        }
      } catch (error) {
    ...
    console.log(
      `🎉 Indexing complete! Total documents indexed: ${totalIndexed}`
    );
  }

  private async createSearchDocument(
    ...
    switch (resource.resourceType) {
      case "Patient":
        ...
        if (resource.birthDate)
          textParts.push(`Birth Date: ${resource.birthDate}`);
        break;

      case "Condition":
        if (resource.code?.text)
          textParts.push(`Condition: ${resource.code.text}`);
        if (resource.code?.coding?.[0]?.display)
          textParts.push(`Diagnosis: ${resource.code.coding[0].display}`);
        if (resource.clinicalStatus?.text)
          textParts.push(`Status: ${resource.clinicalStatus.text}`);
        break;

      case "Observation":
        if (resource.code?.text)
          textParts.push(`Observation: ${resource.code.text}`);
        ...
        if (resource.valueString)
          textParts.push(`Value: ${resource.valueString}`);
        break;

      case "Procedure":
        if (resource.code?.text)
          textParts.push(`Procedure: ${resource.code.text}`);
        if (resource.performedDateTime)
          textParts.push(`Performed: ${resource.performedDateTime}`);
        break;

      case "Encounter":
        if (resource.type?.[0]?.text)
          textParts.push(`Encounter: ${resource.type[0].text}`);
        if (resource.class?.display)
          textParts.push(`Class: ${resource.class.display}`);
        if (resource.period?.start)
          textParts.push(`Date: ${resource.period.start}`);
        break;
    }
