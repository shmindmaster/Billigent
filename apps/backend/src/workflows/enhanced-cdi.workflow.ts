 ...
 */
export async function runEnhancedCDIAnalysis(
  encounterId: string,
  options: {
  ...
  try {
    ...
    // Step 2: Extract clinical evidence from Data Lake
    const clinicalEvidenceResources = await getClinicalEvidence(encounter.encounterId);
    const clinicalEvidence = clinicalEvidenceResources.map(resource => JSON.stringify(resource, null, 2));
 ...
 */
export async function askCDIQuestion(
 ...
 */
export async function generateCDIReport(
...
async function performClinicalAnalysis(encounter: any, clinicalEvidence: string[]) {
...
Current Diagnoses: ${encounter.diagnoses?.map((d: any) => `${d.icdCode}: ${d.description}`).join(', ') || 'None specified'}
Procedures: ${encounter.procedures?.map((p: any) => `${p.cptCode}: ${p.description}`).join(', ') || 'None specified'}

Clinical Evidence:
${clinicalEvidence.join('\n\n')}
...
Current Codes:
- Diagnoses: ${encounter.diagnoses?.map((d: any) => `${d.icdCode}: ${d.description}`).join('\n- ') || 'None'}
...
function categorizeEvidence(clinicalEvidence: string[], analysis: any) {
  return {
    supporting: clinicalEvidence.filter(e => e.includes('support')),
