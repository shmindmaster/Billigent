...
export type AuthorityTier = 'primary' | 'secondary' | 'tertiary' | 'competitive' | 'regulatory' | 'standards';
...
const citationCache: { loaded: boolean; byCanonical: Map<string, NormalizedCitation>; all: NormalizedCitation[] } = {
  loaded: false,
  byCanonical: new Map(),
  all: []
};

const CANON_FILE = path.resolve(process.cwd(), 'docs', 'research', 'normalized_citations.jsonl');
...
const CATEGORY_TIER_MAP: Array<{ test: RegExp; tier: AuthorityTier }> = [
  ...
  { test: /(FHIR|HFMA|MAP Key|MAP Keys|AHIMA|ACDIS|CARIN|Guideline)/i, tier: 'standards' },
...
export function classifyAuthorityTier(entry: RawCorpusEntry): { tier: AuthorityTier; issue?: string } {
...
export function buildNormalized(entry: RawCorpusEntry, timestamp: string): NormalizedCitation {
  const issues: string[] = [];
...
export function loadNormalizedCitations(): NormalizedCitation[] {
  ...
  if (!fs.existsSync(CANON_FILE)) return [];
  const lines = fs.readFileSync(CANON_FILE, 'utf-8').split(/\r?\n/).filter(Boolean);
...
export function findCitationByTitle(title: string): NormalizedCitation | undefined {
...
export function computeCitationCoverage(sourceCitations: string[]): { authoritativeFactCount: number; totalFactCount: number; authoritativePct: number } {
  loadNormalizedCitations();
  const authoritative = new Set<AuthorityTier>(['primary', 'regulatory', 'standards']);
...
export async function normalizeCorpusFile(corpusPath: string, outPath: string): Promise<{ total: number; normalized: number; withIssues: number; byTier: Record<string, number> }> {
  const rl = readline.createInterface({ input: fs.createReadStream(corpusPath, 'utf-8'), crlfDelay: Infinity });
  const out = fs.createWriteStream(outPath, { flags: 'w' });
  const seen = new Set<string>();
  let total = 0; let normalized = 0; let withIssues = 0; const byTier: Record<string, number> = {};
