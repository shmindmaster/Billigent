...
class RAGIndexer {
  ...
  constructor() {
    ...
    this.fileSystemClient = this.dataLakeClient.getFileSystemClient(
      this.container
    );
  ...
  async run(): Promise<void> {
    ...
    const jsonFiles = files.filter(
      (p) => p.endsWith(".json") || p.endsWith(".jsonl")
    );
    ...
    console.log(
      `📂 Found ${
        jsonFiles.length
      } JSON/JSONL file(s) to process: ${jsonFiles.join(", ")}`
    );
    ...
    for (const path of jsonFiles) {
      ...
      if (path.endsWith(".jsonl")) {
        ...
        for (const line of lines) {
          try {
            ...
            console.warn(
              `  Skipping malformed JSONL line: ${(e as Error).message}`
            );
          }
        }
      ...
      for (let i = 0; i < records.length; i += 100) {
        ...
        try {
          ...
          console.error(
            `  ❌ Embedding API failed for batch starting at ${i}: ${
              (e as Error).message
            }`
          );
          continue;
        ...
        try {
          ...
          total += success;
          console.log(
            `  📦 Indexed batch ${Math.floor(i / 100) + 1} (${success}/${
              docs.length
            }) | Total: ${total}`
          );
        } catch (e) {
          console.error(
            `  ❌ Failed to upload batch at offset ${i}: ${
              (e as Error).message
            }`
          );
        }
  ...
  private async listFilesRecursive(dir: any, prefix = ""): Promise<string[]> {
    ...
    return results.map((p) =>
      p.startsWith(this.directory + "/") ? p : `${this.directory}/${p}`
    );
  }
