...
class PhysicianQueryAuditRepository {
  ...
  static getInstance(): PhysicianQueryAuditRepository {
    if (!PhysicianQueryAuditRepository.instance) PhysicianQueryAuditRepository.instance = new PhysicianQueryAuditRepository();
  ...
  private async ensureInit(): Promise<void> {
    ...
    this.container = await cosmos.getOrCreateContainer('physician-query-audit', '/physicianQueryId');
    this.initialized = true;
  }

  async list(physicianQueryId: string): Promise<PhysicianQueryAuditEventRecord[]> {
    await this.ensureInit();
    const querySpec = { query: 'SELECT * FROM c WHERE c.physicianQueryId = @id AND c.type = "physicianQueryAuditEvent" ORDER BY c.sequence ASC', parameters: [{ name: '@id', value: physicianQueryId }] };
    const { resources } = await this.container.items.query(querySpec, { partitionKey: physicianQueryId }).fetchAll();
  ...
  async append(input: AppendInput): Promise<PhysicianQueryAuditEventRecord> {
    ...
    const querySpec = { query: 'SELECT TOP 1 * FROM c WHERE c.physicianQueryId = @id AND c.type = "physicianQueryAuditEvent" ORDER BY c.sequence DESC', parameters: [{ name: '@id', value: physicianQueryId }] };
    const { resources } = await this.container.items.query(querySpec, { partitionKey: physicianQueryId }).fetchAll();
    ...
    await this.container.items.create({ ...record, _partitionKey: physicianQueryId });
    return record;
  }
}
