...
function score(query: string, item: CorpusItem): number {
  const qTerms = query
    ...
    .split(/[^a-z0-9]+/)
    .filter(Boolean);
  const text = `${item.source} ${item.category || ""} ${
    item.notes || ""
  }`.toLowerCase();
  let hits = 0;
...
function synthesizeAnswer(query: string, top: CorpusItem[]): string {
  ...
  const bulletLines = top.map(
    (t) =>
      `- ${t.source}${t.category ? ` [${t.category}]` : ""}: ${t.notes?.slice(
        0,
        180
      )}`
  );
  return [
    ...
    "",
    "Naive Answer:",
    `Based on ${top.length} matching sources, focus on: ${top
      .map((t) => t.category || t.source)
      .slice(0, 3)
      .join(", ")}.`,
  ].join("\n");
}
