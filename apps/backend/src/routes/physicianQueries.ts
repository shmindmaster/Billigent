...
import PhysicianQueryRepository, {
  PhysicianQueryStatus,
  PhysicianQueryEventType,
} from "../repositories/physicianQuery.repository";
  ...
  try {
    ...
    if (!templateCode || !createdById || !assignedPhysicianId) {
      return res
        .status(400)
        .json({
          error: "templateCode, createdById, assignedPhysicianId required",
        });
    }
    ...
    await PhysicianQueryAuditRepository.append({
      physicianQueryId: pq.id,
      eventType: PhysicianQueryEventType.CREATED,
...
router.post("/:id/send", async (req, res) => {
  try {
    ...
    if (pq.status !== PhysicianQueryStatus.DRAFT)
      return res.status(409).json({ error: "Only draft can be sent" });
    const updated = await PhysicianQueryRepository.update(id, {
      status: PhysicianQueryStatus.SENT,
      draft: false,
    ...
    await PhysicianQueryAuditRepository.append({
      physicianQueryId: id,
      eventType: PhysicianQueryEventType.SENT,
      actorUserId: actorUserId || updated.createdById,
      previousStatus: PhysicianQueryStatus.DRAFT,
      newStatus: PhysicianQueryStatus.SENT,
    });
    res.json(updated);
  } catch (e) {
...
router.post("/:id/respond", async (req, res) => {
  try {
    ...
    if (!responderId || !responseType)
      return res
        .status(400)
        .json({ error: "responderId & responseType required" });
    const pq = await PhysicianQueryRepository.get(id);
    if (!pq) return res.status(404).json({ error: "Not found" });
    if (pq.status !== PhysicianQueryStatus.SENT)
      return res
        .status(409)
        .json({ error: "Only sent queries can be responded" });

    // Store response inline for now (future: separate container if needed)
    const updated = await PhysicianQueryRepository.update(id, {
      status: PhysicianQueryStatus.RESPONDED,
      respondedAt: new Date().toISOString(),
      statusUpdatedAt: new Date().toISOString(),
      draft: false,
      freeTextSupplement: responseText || pq.freeTextSupplement,
    });
    if (!updated) return res.status(500).json({ error: "Failed to update" });
    await PhysicianQueryAuditRepository.append({
      physicianQueryId: id,
      eventType: PhysicianQueryEventType.RESPONDED,
      actorUserId: responderId,
      previousStatus: PhysicianQueryStatus.SENT,
      newStatus: PhysicianQueryStatus.RESPONDED,
      metadata: { responseType },
    });
    res.json({
      query: updated,
      response: { responderId, responseType, responseText },
    });
  } catch (e) {
...
router.get("/", async (_req, res) => {
  const { queries } = await PhysicianQueryRepository.list({
    limit: 50,
    page: 1,
  });
  res.json(queries);
...
router.get("/:id/audit", async (req, res) => {
  try {
    ...
    if (events.length === 0)
      return res
        .status(404)
        .json({ error: "No audit events or query not found" });
    // Basic integrity verification: check chain linkage
    const brokenLinks: number[] = [];
    for (let i = 1; i < events.length; i++) {
      if (events[i].prevHash !== events[i - 1].hash)
        brokenLinks.push(events[i].sequence);
    }
    res.json({
      events,
      integrity: brokenLinks.length === 0 ? "OK" : "BROKEN",
      brokenLinks,
    });
  } catch (e) {
